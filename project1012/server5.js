...
var server=http.createServer(function(request,response){
  //아래의 코드들이 동작하기 전에, 서버는 클라이언트가 원하는 작업을 선택하게함
  //요청분석!!!--> 요청 정보를 담고 있는 request객체를 이용해야 한다.
  console.log(request.url);
  //1) 클라이언트가 회원가입 양식을 보기 원하면 아래코드 수행
  //서버에 존재하는 회원가입양식 폼 파일을 읽어서, 클라이언트의 브라우저에 보내주자
  ...
  //3) 이미지를 원하면 이미지 보여줘야 하고,
  //4) 오디오파일을 원하면 오디오를 전송해줘야한다.
});
